...
namespace P2FK.IO.Controllers
{
    ...
    public class GetObjectsOwnedByAddressController : ControllerBase
    {
        ...
        public GetObjectsOwnedByAddressController(Wrapper wrapper)
        {
            _wrapper = wrapper;
        }
        ...
        public async Task<ActionResult> Get(string address, int skip = 0, int qty = -1, bool mainnet = true, bool verbose = false)
        {
            ...
            if (Regex.IsMatch(address, pattern))
            {
                ...
                if (mainnet)
                {
                    arguments = "--versionbyte " + _wrapper.ProdVersionByte + " --getobjectsownedbyaddress --password " + _wrapper.ProdRPCPassword + " --url " + _wrapper.ProdRPCURL + " --username " + _wrapper.ProdRPCUser + " --skip " + skip + " --qty " + qty + " --address " + address;
                ...
                {
                    arguments = "--versionbyte " + _wrapper.TestVersionByte + " --getobjectsownedbyaddress --password " + _wrapper.TestRPCPassword + " --url " + _wrapper.TestRPCURL + " --username " + _wrapper.TestRPCUser + " --skip " + skip + " --qty " + qty + " --address " + address;
                    if (verbose) { arguments = arguments + " --verbose"; }
