...
namespace P2FK.IO.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class GetInquiriesCreatedByAddressController : ControllerBase
    {
        private readonly Wrapper _wrapper;

        public GetInquiriesCreatedByAddressController(Wrapper wrapper)
        {
            _wrapper = wrapper;
        }

        // GET <GetInquiriesCreatedByAddressController>/5
        /// <summary>Get all P2FK inquiries created (initiated) by a given address.</summary>
        ...
        public async Task<ActionResult> Get(string address, int skip = 0, int qty = 10, bool mainnet = true, bool verbose = false)
        {
            ...
            if (Regex.IsMatch(address, pattern))
            {
                ...
                if (mainnet)
                {
                    arguments = "--versionbyte " + _wrapper.ProdVersionByte + " --getinquiriescreatedbyaddress --password " + _wrapper.ProdRPCPassword + " --url " + _wrapper.ProdRPCURL + " --username " + _wrapper.ProdRPCUser + " --skip " + skip + " --qty " + qty + " --address " + address;
                ...
                {
                    arguments = "--versionbyte " + _wrapper.TestVersionByte + " --getinquiriescreatedbyaddress --password " + _wrapper.TestRPCPassword + " --url " + _wrapper.TestRPCURL + " --username " + _wrapper.TestRPCUser + " --skip " + skip + " --qty " + qty + " --address " + address;
                    if (verbose) { arguments = arguments + " --verbose"; }
