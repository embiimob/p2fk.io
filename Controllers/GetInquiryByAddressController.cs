...
namespace P2FK.IO.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class GetInquiryByAddressController : ControllerBase
    {
        private readonly Wrapper _wrapper;

        public GetInquiryByAddressController(Wrapper wrapper)
        {
            _wrapper = wrapper;
        }
        ...
        public async Task<ActionResult> Get(string address, bool mainnet = true, bool verbose = false)
        {
            ...
            if (Regex.IsMatch(address, pattern))
            {
                ...
                if (mainnet)
                {
                    arguments = "--versionbyte " + _wrapper.ProdVersionByte + " --getinquirybyaddress --password " + _wrapper.ProdRPCPassword + " --url " + _wrapper.ProdRPCURL + " --username " + _wrapper.ProdRPCUser + " --address " + address;
                ...
                else { arguments = "--versionbyte " + _wrapper.TestVersionByte + " --getinquirybyaddress --password " + _wrapper.TestRPCPassword + " --url " + _wrapper.TestRPCURL + " --username " + _wrapper.TestRPCUser + " --address " + address;
                    if (verbose) { arguments = arguments + " --verbose"; }
